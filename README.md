...
| | |
|---|---|
| Zodiac | Sidereal, Lahiri (Chitrapaksha) by default; Raman, KP and Fagan-Bradley also offered |
...
rates for the planets (Standish, JPL), Meeus ch.21 precession and the IAU 1980
nutation series, and Espenak-Meeus Delta T.

The two-body element fit misses the Jupiter-Saturn "great inequality", which left
Saturn up to 7.4 arcminutes out, enough to move a graha across a pada boundary
and so change its reported pada and navamsa sign. `data/perturbations.js` is a
sampled residual table that removes it. Worst-case error against JPL Horizons,
measured at epochs from 1950 to 2050 by `test/test.js`:

| Sun | Moon | Mercury | Venus | Mars | Jupiter | Saturn |
|---|---|---|---|---|---|---|
| 12″ | 15″ | 16″ | 13″ | 35″ | 2″ | 1″ |

A nakshatra pada is 12,000 arcseconds wide, so signs, nakshatras, padas and
divisional charts are all well clear of the error. The correction table covers
1800 to 2100; outside that range Jupiter and Saturn fall back to the raw
Keplerian result and the chart says so.

For independent confirmation, the test suite reproduces every published Vedic
transit date it is given: Saturn into Kumbha on 17 January 2023, Jupiter into
Vrishabha on 1 May 2024, Saturn into Meena on 29 March 2025, Jupiter into Mithuna
on 14 May 2025, Mesha Sankranti on 14 April 2025, Makara Sankranti on 14 January
2026, and mean Rahu into Meena on 30 October 2023.

The ascendant is checked a different way, against two identities that need no
reference data at all: at geometric sunrise the Sun's longitude *is* the
ascendant, and at local apparent noon it is the Midheaven. Both hold to under an
arcsecond from the equator to 64°N.
...
data/perturbations.js   Jupiter/Saturn residual correction table
scripts/                regenerate the two data files
...
# Jupiter/Saturn corrections: geometric heliocentric vectors, J2000 ecliptic,
# Sun-centred, 100-day steps, for each of 599 (Jupiter) and 699 (Saturn)
...
&START_TIME='JD2378496.5'&STOP_TIME='JD2488496.5'&STEP_SIZE='100d'" \
  | sed -n '/\$\$SOE/,/\$\$EOE/p' | sed '1d;$d' > vec-jupiter.csv
...
- **Ayanamsa variants.** Lahiri is the one validated above. Raman, KP and
  Fagan-Bradley are implemented as their published J2000 values plus accumulated
  precession, and are accurate to a few arcminutes rather than exactly.
